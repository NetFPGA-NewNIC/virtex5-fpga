...
void rx_wq_function(struct work_struct *wk)
{
    ...
    {
        //printk(KERN_INFO "Myd: polling_counter: %d\n", polling_counter);
    ...
    dump:
    huge_page_status = *((u64 *)current_hp_addr);            //QW0 contains this information
    printk(KERN_ERR "Myd: huge_page_status: 0x%08x %08x\n", (u32)(huge_page_status >>32), (u32)huge_page_status);
    printk(KERN_ERR "Myd: current_pkt_len: 0x%08x\n", current_pkt_len);
    printk(KERN_ERR "Myd: current_pkt_dw_index: 0x%08x\n", my_drv_data->current_pkt_dw_index);
    printk(KERN_ERR "Myd: next_pkt_dw_index: 0x%08x\n", next_pkt_dw_index);
    printk(KERN_ERR "Myd: next_pkt_len: 0x%08x\n", next_pkt_len);
    //printk(KERN_ERR "Myd: pkts: 0x%08x\n", pkts);
    int i, aux;
    aux = my_drv_data->current_pkt_dw_index-32;
    for (i=0;i<220;i++)
    {
        printk(KERN_ERR "qw: %08x : 0x%08x %08x\n", aux, (u32)(current_hp_addr[aux+1]), (u32)current_hp_addr[aux]);
        aux +=2;
    }
    huge_page_status = *((u64 *)current_hp_addr);            //QW0 contains this information
    printk(KERN_ERR "Myd: huge_page_status: 0x%08x %08x\n", (u32)(huge_page_status >>32), (u32)huge_page_status);
