...
int
rx_init(struct my_driver_host_data *my_drv_data)
{
    int i, j;
    for (i = 0; i < RX_HUGE_PAGE_COUNT; i++)
    {
        my_drv_data->rx.huge_page_kern_addr[i] = pci_alloc_consistent(
            my_drv_data->pdev, HUGE_PAGE_SIZE, &my_drv_data->rx.huge_page_dma_addr[i]);
    ...
    // Rx. pci_alloc_consistent zeros the memory. This is not mandatory
    for (i = 0; i < RX_HUGE_PAGE_COUNT; i++)
...
err_01:
    for (j = 0; j < i; j++)
        pci_free_consistent(my_drv_data->pdev, HUGE_PAGE_SIZE,
            my_drv_data->rx.huge_page_kern_addr[j], my_drv_data->rx.huge_page_dma_addr[j]);
    return -1;
}

void
rx_release(struct my_driver_host_data *my_drv_data)
{
    int j;
